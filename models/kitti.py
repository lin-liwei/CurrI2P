...
class kitti_pc_img_dataset(data.Dataset):
    ...
    def search_for_accumulation(self, pc_folder, seq_pose_folder,
        ...
        while len(pc_np_list) < 3:
            ...
            pc_np = npy_data[0:3, :]
            intensity_np = npy_data[3:4, :]
            sn_np = npy_data[4:7, :]

            P_oj = np.load(os.path.join(seq_pose_folder, '%06d.npz' % seq_j))['pose'].astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        return {'img': torch.from_numpy(img.astype(np.float32) / 255.).permute(2, 0, 1).contiguous(),
                ...
                'pc_kpt_idx': torch.from_numpy(pc_kpt_idx),
                'pc_outline_idx':torch.from_numpy(pc_outline_idx),
                'img_kpt_idx':torch.from_numpy(img_kpt_index).long() ,
